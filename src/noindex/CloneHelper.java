...
public class CloneHelper {
		...
		for (Bag bagInSetA : setA) {
			...
			for (Bag bagInSetB : setB) {
				if (bagInSetA.getId() != bagInSetB.getId()) {
					if (bagInSetA.getId() != bagInSetB.getId()) {
						// if (bagInSetA.getId() < bagInSetB.getId()) - sarah
	...
	// function Id and bag Id are the same - sarah
...
//			String functionId =  functionIdAndBagId[0];
	...
	public QueryBlock deserialiseToQueryBlock(String s, List<Entry<String, TokenInfo>> listOfTokens) throws ParseException {
		...
		if (null != s && s.trim().length() > 0) {
			String[] bagAndTokens = s.split("@#@");
//			String[] functionIdAndBagId = bagAndTokens[0].split(","); same change as in serialise - Sarah
//			String functionId =  functionIdAndBagId[0];
			String bagId = bagAndTokens[0];
			String functionId = bagId;
			...
			QueryBlock queryBlock = new QueryBlock(bagId,queryBlockSize);
			return queryBlock;
		}
		throw new ParseException("parsing error", 0);
	}

	private int parseAndPopulateQueryBlock(List<Entry<String, TokenInfo>> listOfTokens,String inputString) {
